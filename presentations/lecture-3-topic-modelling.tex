...
\begin{frame}{LDA Intuition: The Generative Story}
    ...
    \begin{tikzpicture}[node distance=0.8cm and 1.8cm]
        ...
        \node[gen step, right=of step1] (step2) {2. Pick a topic};
        \node[gen step, right=of step2] (step3) {3. Pick a word\\from that topic};
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.48\textwidth}
            ...
            \begin{tikzpicture}[yscale=0.45]
                % Horizontal bar chart (descending probability)
                \fill[sgsssMagenta!70] (0,5) rectangle (3.0,5.7);
                \fill[sgsssMagenta!70] (0,4) rectangle (2.5,4.7);
                \fill[sgsssMagenta!70] (0,3) rectangle (2.0,3.7);
                \fill[sgsssMagenta!70] (0,2) rectangle (1.6,2.7);
                \fill[sgsssMagenta!70] (0,1) rectangle (1.2,1.7);
                \fill[sgsssMagenta!70] (0,0) rectangle (0.9,0.7);

                % Axes
                \draw[thick] (0,0) -- (3.5,0);
                \draw[thick] (0,0) -- (0,6.2);

                % Word labels on y-axis
                \node[left, font=\scriptsize] at (0,5.35) {health};
                \node[left, font=\scriptsize] at (0,4.35) {NHS};
                \node[left, font=\scriptsize] at (0,3.35) {hospital};
                \node[left, font=\scriptsize] at (0,2.35) {patients};
                \node[left, font=\scriptsize] at (0,1.35) {care};
                \node[left, font=\scriptsize] at (0,0.35) {funding};

                % Axis label
                \node[left, font=\scriptsize] at (0,6.2) {$\phi$};

                % Title
                \node[above, font=\scriptsize\bfseries] at (1.75,6.5) {Topic 1 (Health)};
            \end{tikzpicture}
            \end{center}
...
\begin{frame}{Example: Topics from Parliamentary Debates}
    \small
    {\footnotesize LDA with \(K=4\) topics fitted to UK House of Commons debates. Top 5 words per topic:}

    \vspace{0.2cm}
    \begin{tabular}{@{}cl@{}}
        \toprule
        \textbf{Topic} & \textbf{Top 5 Words} \\
        \midrule
        \textbf{\color{sgsssMagenta} 1 --- Health} & health, NHS, hospital, patients, care \\[0.15cm]
        \textbf{\color{sgsssGreen} 2 --- Education} & education, school, university, students, teachers \\[0.15cm]
        \textbf{\color{sgsssBlack} 3 --- Economy} & economy, growth, tax, business, employment \\[0.15cm]
        \textbf{\color{sgsssMagenta!60!sgsssBlack} 4 --- Security} & security, defence, military, police, terrorism \\
        \bottomrule
    \end{tabular}
...
\begin{frame}{Interpreting and Validating Topics}
    ...
    \begin{itemize}\setlength\itemsep{0pt}
        \item \faIcon{trash-alt} \textbf{Junk topics} --- stopwords, boilerplate
        \item \faIcon{expand-arrows-alt} \textbf{Over-splitting} --- one theme spread across several topics
        \item \faIcon{exclamation-triangle} \textbf{Label bias} --- reading meaning into vague word lists
    \end{itemize}
\end{frame}
    ...
    \begin{columns}[T]
        ...
        \begin{column}{0.44\textwidth}
            ...
            \begin{tikzpicture}[node distance=0.6cm]
                ...
                \node[stm node, below=0.5cm of prev, xshift=1.05cm] (topics) {Discovered\\topics};
